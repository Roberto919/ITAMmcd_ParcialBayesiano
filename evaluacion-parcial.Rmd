...
Ahora buscamos incorporar un intervalo de confianza de la recta de regresión que condicione en la mediana del coeficiente de la variable que se queda fuera del modelo.
Buscamos obtener la distribución posterior del intercepto y edad.centrada  ( o intercepto y peso.centrado) dado que peso.centrado toma el valor mediano ( o edad.centrada toma el valor mediano). 

Para obtener la distribución marginal conjunta del intercepto y el coeficiente condicionando sobre el otro coeficiente, se filtran las observaciones de la simulación de la distribución posterior tal que el coeficiente tome un valor igual a la mediana y  +/- un pequeño margen. Este margen se calcula ya que dada la muestra, la probabilidad de que el valor mediano ocurra de forma exacta es 0. Una vez filtradas las observaciones que cumplan esta condición, remuestreamos sobre el intercepto y coeficiente de itnerés de forma conjunta, y evaluamos para distintos valores de la variable. De esta manera, logramos construir intervalos de confianza de la variable altura condicionando en el valor de uno de los coeficientes. 


```{r}
cond_edad<-as_tibble(fit_1b)%>% filter(edad.centrada>=(2.33-.01) & edad.centrada<=(2.33+.01))
cond_peso<-as_tibble(fit_1b)%>% filter(peso.centrado>=(1.43-.01) & peso.centrado<=(1.43+.01))

sim<-function(df,var,min,max){
  sim_df<-matrix(nrow=200,ncol=0)
  for(xvar in seq(min,max,.5)){
    hist<-c()
    for(i in seq(1,200)){
      x<-sample_n(df,1)
      val<-x[["(Intercept)"]]+x[[var]]*xvar
      hist<-c(hist,val)
    
    }
    sim_df<-cbind(sim_df,hist)
  }
  colnames(sim_df)<-seq(min,max,.5)
  return(sim_df)
}

peso<-sim(cond_edad,"peso.centrado",-20,30)
edad<-sim(cond_peso,"edad.centrada",-10,10)
```


```{r}  
peso_range<-seq(-20,30,.5) 
peso_sum<-matrix(nrow=0,ncol=3)  
for (i in seq(1,ncol(peso))){
  
  peso_sum<-rbind(peso_sum,c(peso_range[i],quantile(peso[,i],.055)[[1]], quantile(peso[,i],.945)[[1]]))
  
  
}

edad_range<-seq(-10,10,.5)  
edad_sum<-matrix(nrow=0,ncol=3)  
for (i in seq(1,ncol(edad))){
  
  edad_sum<-rbind(edad_sum,c(edad_range[i],quantile(peso[,i],.055)[[1]], quantile(peso[,i],.945)[[1]]))
  
  
}

colnames(edad_sum)<-c("edad.centrada","altura_low","altura_high")
colnames(peso_sum)<-c("peso.centrado","altura_low","altura_high")

edad_sum<-as_tibble(edad_sum)
peso_sum<-as_tibble(peso_sum)
```


```{r}  
edad_sum<-cbind(edad_sum,c(coef(fit_1b)[[1]]+coef(fit_1b)[[3]]*edad_sum[,1]))
peso_sum<-cbind(peso_sum,c(coef(fit_1b)[[1]]+coef(fit_1b)[[2]]*peso_sum[,1]))

colnames(edad_sum)<-c("edad.centrada","altura_low","altura_high","altura_mean")
colnames(peso_sum)<-c("peso.centrado","altura_low","altura_high","altura_mean")

 g_points_edad <-geom_point(data= menores.edad, aes(edad.centrada,altura),alpha=.1)
  g_1a<-as_tibble(edad_sum) %>%
  ggplot(aes(edad.centrada,altura_mean)) + #graficamos peso vs altura_prom
  geom_ribbon(aes(ymin=altura_low,ymax=altura_high),fill = "grey70",alpha=.3)+ #bandas de predicción
  geom_line(color="red") + #media de predicciones
  ggtitle("Altura vs respuesta 1")+
  ylab("altura")
  
 g_points_peso <-geom_point(data= menores.edad, aes(peso.centrado,altura),alpha=.1)
  g_1a_2<-as_tibble(peso_sum) %>%
  ggplot(aes(peso.centrado,altura_mean)) + #graficamos peso vs altura_prom
  geom_ribbon(aes(ymin=altura_low,ymax=altura_high),fill = "grey70",alpha=.3)+ #bandas de predicción
  geom_line(color="red") + #media de predicciones
  ggtitle("Altura vs respuesta 2")+
  ylab("altura")
```
