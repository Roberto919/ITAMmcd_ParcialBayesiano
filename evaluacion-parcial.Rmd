...
Ahora utilizaremos la función posterior_predict() para calcular las predicciones de altura con base en los pesos de los 5 individuos bajo nuestro modelo de regresión ya que considera la incertidumbre asociada al error observacional (epsilon) incorporando sigma o el error aditivo.

```{r}
post_predict_1a<-as_tibble(posterior_predict(fit_1a, newdata=new,seed=SEED)) %>% 
  #cada fila es una simulación bajo una muestra distinta
  mutate(sample_id = 1:4000) %>% #creamos ID para cada
  pivot_longer(cols = 1:5) %>%
  mutate(individuo = fct_inorder(name)) %>% 
  group_by(individuo) %>% #es la predicción de cada individuo (1-5)
...
g_1a<-post_predict_1a %>%
  ...
  geom_jitter(data=howell %>% filter(peso>40),
              aes(peso,altura),
...
```{r }
y_linpred <- posterior_linpred(fit_1a, newdata=new,seed=SEED) 
#predice únicamente a partir de la variación alrededor de la media
#son las predicciones de la distribución posterior incorporando el nivel de incertidumbre asociado al error de medición
#incertidumbre asociada a los coeficientes únicamente
#tenemos nuestra nueva variable, se multiplica por los coeficientes
summary(y_linpred)
```
...
head(menores_edad)
```
Para poder dar una interpretación más sencilla al modelo lineal para predecir la altura, centramos las variables alrededor de su media:
...
De los resultados anteriores, vemos que el intercepto representa la altura promedio de los menores de 18 años para una persona con un peso y edad alrededor del promedio la cual corresponde a 108 cm o 1.08 metros. Por otro lado, el peso y edad centrada tienen un coeficiente positivo en el modelo, lo cual indica para ambos predictores un incremento sobre el valor promedio centrado del contribuye a un incremento en la predicción de la altura. En el caso del peso.centrado, dicho incremento aumenta 1.4 cm la altura y la edad, tiene una mayor aportación al pronóstico sumando 2.3cm adicionales.
...
```{r}
#howell.centrado
m_1d <- stan_glm(altura ~ log(peso), data = howell, seed=SEED, refresh = 0)
#prior_summary(m_1a)
summary(m_1d)
```
...
```{r}
poly(edad.centrado,1,raw=TRUE)
poly(edad.centrado,2,raw=TRUE)
poly(edad.centrado,3,raw=TRUE)
poly(edad.centrado,4,raw=TRUE)
poly(edad.centrado,5,raw=TRUE)
poly(edad.centrado,6,raw=TRUE)
```
...
Cuando utilizamos los criterios de información estamos midiendo su capacidad predictiva, qué tan bien predice el modelo en datos que no has observado: validación cruzada, WAIC (decir cómo le va al modelo para predecir en un futuro). Cuál es el mejor modelo (devianza)
...
compara la
estimación de número efectivo de parámetros (cómo cambia)
